...
[https://github.com/jjtt1917/Tinoco-Rodriguez_Jose_201_A06](https://github.com/jjtt1917/Tinoco-Rodriguez_Jose_201_A06)
...
```{r}
player_info$LJ[[4]]#indexing
player_info$LJ$points #names
```
...
for (player in names(player_info)) {
  points_pg <- player_info[[player]]$points / player_info[[player]]$games_played
  points_per_game[[player]] <- points_pg
}
...
```{r}
rounded_points_pg <- sapply(points_per_game, function(x) round(x, 2))
print(rounded_points_pg)
```
...
round_mat <- apply(mat_rand, 1:2, round_func)
print(round_mat)
```
...
```{r}
avg_temps <- apply(max_temps[, 3:14], 2, function(x) mean(x, na.rm = TRUE))
avg_temps_df <- data.frame(
  month = colnames(max_temps) [3:14],
  average_temperature = avg_temps
)
...
for (state in state_list) {
  state_df <- max_temps[max_temps$state == state, ]
  if (state == "CO") {
    CO_df <- state_df
  } else if (state == "OH") {
    OH_df <- state_df
...
```{r}
CO_df$median_temp <- apply(CO_df[, 3:14], 1, median)

year_max_median <- CO_df$year[which.max(CO_df$median_temp)]
year_min_median <- CO_df$year[which.min(CO_df$median_temp)]

sentence <- paste0("The maximum median annual temperature of ", CO_df$median_temp[which.max(CO_df$median_temp)],
                   " occurred in ", year_max_median,
                   " and the minimum median annual temperature of ", CO_df$median_temp[which.min(CO_df$median_temp)],
                   " occurred in ", year_min_median, ".")
print(sentence)
```
